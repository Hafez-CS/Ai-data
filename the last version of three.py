...
if not hf_token or hf_token == "hf_YourTokenHere":
    raise ValueError("HF_TOKEN not found or is empty. Please set your Hugging Face token.")

def financial_predictor(file_path: str, target_column: Optional[str] = None):
    """
    Process a financial dataset and perform regression predictions using Hugging Face API.
    
    Args:
        file_path (str): Path to the CSV or Excel file
        target_column (str, optional): Target column for prediction. If None, HF will select one.
    
    Returns:
        dict: Prediction results including message, target column, model used, HF recommendation,
    ...
    class Predictor:
        def __init__(self, data_processor):
            ...
            try:
                ...
                self.model = "meta-llama/Llama-3.1-8B-Instruct"  # مدل پیش‌فرض Hugging Face
            except Exception as e:
                raise ValueError(f"Error connecting to Hugging Face API: {str(e)}")

        def analyze_dataset_with_hf(self, df, user_target_column=None):
            try:
                ...
                return None, None, f"Error analyzing dataset with Hugging Face API: {str(e)}"

        def train_and_predict(self, user_target_column=None):
            if self.data_processor.df is None:
                raise ValueError("Please load a CSV or Excel file first.")

            recommended_target, recommended_model, recommendation_text = self.analyze_dataset_with_hf(
                self.data_processor.df, user_target_column
            )
            if not recommended_target or not recommended_model:
                raise ValueError(f"Hugging Face could not recommend a suitable target or model: {recommendation_text}")
            ...
            try:
                ...
                ax.plot(np.arange(len(y_test), len(y_test) + 5), future_pred, color='green', linestyle='--',
                        label=get_display(arabic_reshaper.reshape('پیش‌بینی‌های آینده')), linewidth=2)
                ...
                response_data = {
                    ...
                    "hf_recommendation": recommendation_text,
    ...
    try:
        ...
        data_processor.load_file(file_path)
