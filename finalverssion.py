...
class DataProcessor:
    ...
    async def load_file_from_db(self, chat_id: int):
        ...
        if not result:
            raise HTTPException(status_code=404, detail="هیچ فایل پردازش‌شده‌ای یافت نشد.")
...
async def upload_file(file: UploadFile, chat_id: int = Form(...)):
    ...
    return await data_processor.load_file(file, file.filename, chat_id)
...
async def predict(request: PredictRequest):
    return await predictor.train_and_predict(request.chat_id)
...
async def predict_with_target(request: PredictRequest):
    ...
    return await predictor.train_and_predict(request.chat_id, target_column_cleaned)
